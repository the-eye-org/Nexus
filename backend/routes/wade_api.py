...
import os
# Create a Blueprint
wade_bp = Blueprint('wade_api', __name__, template_folder='templates')
...
# Initialize DB (Run this once or inside the blueprint setup)
def init_db():
    if not os.path.exists(DB_PATH):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("CREATE TABLE menu (id INTEGER PRIMARY KEY, item TEXT, price TEXT, description TEXT)")
        c.execute("CREATE TABLE secrets (id INTEGER PRIMARY KEY, flag TEXT)")
        items = [("chimichanga", "$9.99", "Wade's favorite."), ("taco", "$4.99", "Boring.")]
        c.executemany("INSERT INTO menu (item, price, description) VALUES (?, ?, ?)", items)
        c.execute("INSERT INTO secrets (flag) VALUES (?)", ("flag{w4d3_l0v3s_ch1m1ch4ng4s_4nd_sqli}",))
        conn.commit()
        conn.close()
...
@wade_bp.route("/")
def serve_shack():
    # This serves the HTML page you provided
    return Response("""
<!DOCTYPE html>
<html lang="en">
<head>
...
async function fetchRecipe() {
    ...
    try {
        const res = await fetch('/nex-backend/wade/api/recipe?token=' + encodeURIComponent(token));
...
async function fetchVault() {
    ...
    try {
        const res = await fetch('/nex-backend/wade/api/vault?item=' + encodeURIComponent(item));
        const data = await res.json();
...
@cross_origin()
@wade_bp.route("/api/docs")
def api_docs():
    sample_token = jwt.encode({"user": "customer_bob", "role": "customer"}, JWT_SECRET, algorithm="HS256")
    return jsonify({
        "endpoints": [{"path": "/api/recipe", "sample_token": sample_token}],
        "hint": "The JWT secret is Wade's name."
    })
@cross_origin()
@wade_bp.route("/api/recipe")
def recipe():
